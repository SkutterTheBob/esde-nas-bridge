...
| 2026-08-16 | prune-removed config-drift warning | Fixed -- confirmed with a fixture (real near-miss prompted it) | Real near-miss on `n64`: `.7z` was removed from `n64`'s `extensions:` in config.yaml (unrelated to this session -- ares doesn't support `.7z`, one game had been manually extracted to `.z64` to test), then `scan --system n64` silently stopped seeing all 500 remaining `.7z` ROMs (still genuinely present on the NAS, untouched) since they no longer matched the extension filter. `prune-removed` then flagged all 500 as "stale," indistinguishable by timestamp alone from a real NAS removal -- confirmed via direct DB query (500 `.7z` roms all shared one older `last_seen_at`, the 1 new `.z64` had the latest). No data was actually lost: `prune-removed` is dry-run by default and the user hadn't passed `--apply` -- confirmed via direct filesystem check that all 501 stub files and gamelist.xml entries were still intact. Added a per-ROM check in `prune-removed`: if a stale ROM's extension isn't in its system's *current* `extensions:` list, flag it distinctly ("NOT confirmed missing from the NAS") and require an extra explicit `click.confirm` before `--apply` touches ANY of the batch (not just the flagged ones -- an all-or-nothing gate, simpler and safer than partially applying). Verified with a fixture (real n64 data had since self-resolved -- extension was added back and re-scanned): dry run correctly flags the drift ROM and explains why; `--apply` answering "n" aborts with nothing removed (DB checked directly); answering "y" removes exactly the flagged entry; a second fixture with two genuinely-stale same-extension ROMs and `--apply < /dev/null` (no stdin available) completed with no prompt and no hang, confirming the confirmation gate only triggers when drift is actually present. |
|      |      |        |       |
|      |      |        |       |
